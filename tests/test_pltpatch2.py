"""
PLT Patch v2: More targeted approach.
- Only patch string comparison + dl_iterate_phdr + abort in libzyte PLT
- Leave file I/O (open/fopen/read) intact so Promon can do legitimate work
- Rely on global Interceptor hooks for maps filtering
"""
...
// ── TARGETED PLT patches: only string comparison + phdr + abort ──
// Leave open/fopen/read/fgets/fread INTACT so Promon can do legitimate I/O
var PLT_PATCHES = {
    ...
    'dl_iterate_phdr':  { rva: 0x2fdfa0, patch: [0x31, 0xC0, 0xC3] },          // return 0 (no modules)
    'abort':            { rva: 0x2ff110, patch: [0xEB, 0xFE] },                // jmp $-2 (safety net)
...
function patchLibzyte(base) {
    send("[PATCH] Patching libzyte.so PLT at base " + base);
...
// ── dlopen hook for libzyte detection ──
        ...
        (function(dname) {
            Interceptor.attach(da, {
                ...
                onLeave: function(retval) {
                    if (zytePatched || !this.path) return;
                    if (this.path.indexOf("zyte") !== -1) {
                        send("[DLOPEN] " + this.path + " loaded");
                        try {
                        ...
                        } catch(e) { send("[DLOPEN] ERROR: " + e.message); }
                    }
...
// ── Anti-kill (safe, non-noreturn functions only) ──
...
send("[HOOK] kill/raise/tgkill replaced");
...
if (_dlAddr) {
    ...
    send("[PATCH] " + pc + " linker names renamed");
}

// ── Global stealth hooks (Interceptor.attach on libc - these filter selectively) ──
...
if (closeAddr) {
    Interceptor.attach(closeAddr, { onEnter: function(a) { try { var fd=a[0].toInt32(); if(trackedFds[fd]) delete trackedFds[fd]; } catch(e){} } });
}

// Global strstr hook (selective - only blocks frida needles)
...
// dl_iterate_phdr filter (for non-libzyte callers)
...
send("[READY] Targeted PLT + global stealth. Resuming...");
...
scr = sess.create_script(PLT_PATCH2_JS)
...
for i in range(120):
    ...
    if e % 10 == 0:
        try:
            ...
            log(f"+{e}s alive | patched={r['patched']} patches={r['patches']} | mods={mods}")
