...
`;
  ...
  // java_guard.ts
  var SHIELD_PACKAGE = "pkbpqnzc.";
  var protectedPid = Process.id;
  var sentinelFaultCount = 0;
  var reportGuardInstalled = false;
  var reportGuardInstalling = false;
  var guardStatus = {
    installed: false,
  ...
  function installReportGuardFor(loader) {
    if (reportGuardInstalled || reportGuardInstalling)
      return;
    reportGuardInstalling = true;
    try {
      const factory = frida_java_bridge_default.ClassFactory.get(loader);
      const Report = factory.use("pkbpqnzc.aG");
      const report = Report.a.overload("java.lang.String");
      report.implementation = function(code3) {
        guardStatus.suppressed = true;
        guardStatus.className = "pkbpqnzc.aG";
        guardStatus.message = code3 === null ? "" : code3.toString();
        guardStatus.detail = "direct protection report suppressed";
        send(`[JAVA-GUARD] protection report suppressed: ${code3}`);
        try {
          frida_java_bridge_default.vm.getEnv().exceptionClear();
        } catch (_) {
        }
      };
  ...
  function discoverReportGuard() {
    if (reportGuardInstalled || !frida_java_bridge_default.available)
      return;
  ...
  function installSentinelFaultGuard() {
    Process.setExceptionHandler((details) => {
      try {
        const memory = details.memory;
        if (details.type !== "access-violation" || memory === void 0 || memory.address.compare(ptr("0x7fff0000")) < 0 || memory.address.compare(ptr("0x80010000")) > 0 || sentinelFaultCount >= 64) {
          return false;
        }
        sentinelFaultCount++;
        ...
        const location = module === null ? "unknown" : `${module.name}+${faultPc.sub(module.base)}`;
        send(`[FAULT-GUARD] sentinel #${sentinelFaultCount} address=${memory.address} pc=${faultPc} op=${memory.operation} at=${location} insn=${instruction}`);
        if (faultPc.equals(memory.address) && Process.arch === "x64") {
          ...
          if (Process.arch === "x64" && context.rax !== void 0) {
            context.rax = ptr(0);
          }
  ...
  function installShieldExceptionGuard() {
    ...
    frida_java_bridge_default.performNow(() => {
      try {
        ...
        loadClassSimple.implementation = function(name) {
        ...
        const BaseDexClassLoader = frida_java_bridge_default.use("dalvik.system.BaseDexClassLoader");
        ...
        const KillApplicationHandler = frida_java_bridge_default.use("com.android.internal.os.RuntimeInit$KillApplicationHandler");
        const uncaughtException = KillApplicationHandler.uncaughtException.overload("java.lang.Thread", "java.lang.Throwable");
        uncaughtException.implementation = function(thread, error) {
          let className = "";
          let message = "";
          try {
          ...
          if (className.startsWith(SHIELD_PACKAGE)) {
            ...
            guardStatus.detail = "protection exception returned without invoking app kill";
            try {
              frida_java_bridge_default.vm.getEnv().exceptionClear();
            } catch (_) {
            }
  ...
  discoverReportGuard();
  var directGuardStartedAt = Date.now();
  var directGuardTimer = setInterval(() => {
    try {
      discoverReportGuard();
    } catch (error) {
      guardStatus.detail = `direct report guard retry failed: ${error}`;
    }
    if (reportGuardInstalled || Date.now() - directGuardStartedAt > 1e4) {
