...
`;
  ...
  function ensureLog() {
    ...
    try {
      ...
      if (p)
        logwrite = new NativeFunction(p, "int", ["int", "pointer", "pointer"]);
      if (logTag === null)
        logTag = Memory.allocUtf8String("QUAGOPROBE");
    } catch (_) {
    }
  }
  function log(m) {
    ...
    try {
      ensureLog();
      if (logwrite)
        logwrite(4, logTag, Memory.allocUtf8String(String(m)));
    } catch (_) {
    }
  }
  var blockOn = true;
  var spoofOn = false;
  var blockInstalled = false;
  var blockedCount = 0;
  var gameState = { segment: "", _updated: "0" };
  var stateVer = 0;
  var accelHooked = false;
  var accelLogged = 0;
  var qmHooked = false;
  var rootHooked = false;
  var sensorLogged = false;
  var bx = 0.6;
  var by = 4.2;
  var phase = 0;
  var moveTtl = 0;
  ...
  function hookRoot() {
    ...
    try {
      ...
      if (mod === null)
        return;
      const fn = mod.getExportByName("Java_com_quago_mobile_sdk_root_QuagoRootDetectionNative_checkForRoot");
      Interceptor.attach(fn, {
        onLeave(r) {
          log("native checkForRoot() -> " + r + " (nonzero=rooted)");
        }
      });
      rootHooked = true;
  ...
  function hookSensorLog() {
    if (sensorLogged)
      return;
    try {
      const SM = frida_java_bridge_default.use("android.hardware.SensorManager");
      SM.registerListener.overloads.forEach((ov) => {
        ov.implementation = function(...args) {
          try {
            let s = null, rate = -1, l = null;
            for (const a of args) {
              const cn = a && a.$className;
              if (cn === "android.hardware.Sensor")
                s = a;
              else if (typeof a === "number" && rate < 0)
                rate = a;
              else if (cn && l === null && cn.indexOf("Sensor") < 0)
                l = a;
            }
            const who = l ? l.$className : "?";
            if (who.indexOf("quago") >= 0)
              log("QUAGO subscribes sensor=" + (s ? s.getName() + " type=" + s.getType() : "?") + " rate=" + rate + " listener=" + who);
          } catch (_) {
          }
  ...
  function hookBlock() {
    ...
    try {
      ...
      URL.openConnection.overloads.forEach((ov) => {
      ...
      log("block hook err: " + e);
    }
  }
  function hookAccel(factory) {
    if (accelHooked)
      return;
    try {
      const E2 = factory.use("com.quago.mobile.sdk.e");
      E2.onSensorChanged.overload("android.hardware.SensorEvent").implementation = function(event) {
        if (spoofOn) {
          try {
            ...
            if (arr && arr.length >= 3) {
              ...
              if (accelLogged < 3) {
                accelLogged++;
                log("accel orig=[" + o0.toFixed(2) + "," + o1.toFixed(2) + "," + o2.toFixed(2) + "] -> spoof=[" + s[0].toFixed(2) + "," + s[1].toFixed(2) + "," + s[2].toFixed(2) + "]");
              }
              arr[0] = s[0];
              arr[1] = s[1];
              arr[2] = s[2];
  ...
  function hookQuagoManager(factory) {
    if (qmHooked)
      return;
    try {
      const QM = factory.use("com.supercell.titan.QuagoManager");
      try {
        QM.beginSegment.overloads.forEach((ov) => {
          ov.implementation = function(seg) {
            try {
              gameState.segment = String(seg);
              gameState._updated = String(++stateVer);
              log("segment -> " + seg);
            } catch (_) {
            }
      ...
      try {
        QM.endSegment.overloads.forEach((ov) => {
      ...
      try {
        QM.setKeyValue.overload("java.lang.String", "java.lang.String").implementation = function(k, v) {
          try {
            gameState[String(k)] = String(v);
            gameState._updated = String(++stateVer);
          } catch (_) {
          }
      ...
      try {
        QM.enable.overloads.forEach((ov) => {
          ov.implementation = function() {
  ...
  var timer = setInterval(() => {
    ...
    frida_java_bridge_default.performNow(() => {
      ...
      if (!qmHooked || !accelHooked) {
        const loaders = frida_java_bridge_default.enumerateClassLoadersSync();
        for (const loader of loaders) {
          let f;
          try {
            f = frida_java_bridge_default.ClassFactory.get(loader);
          } catch (_) {
            continue;
          }
          hookQuagoManager(f);
      ...
      if (qmHooked && blockInstalled) {
        log("quago module settled (block armed; tries=" + tries + ")");
        clearInterval(timer);
      } else if (tries > 300) {
        log("quago module partial (qm=" + qmHooked + " block=" + blockInstalled + " accel=" + accelHooked + ")");
        clearInterval(timer);
      }
    });
  }, 250);
  rpc.exports = {
    ...
    setBlock(on) {
      blockOn = !!on;
      log("upload block -> " + blockOn);
      return blockOn;
    },
    setSpoof(on) {
      spoofOn = !!on;
      log("accel spoof -> " + spoofOn);
      return spoofOn;
    },
    status() {
      return { blockOn, blockedCount, spoofOn, accelHooked, qmHooked, blockInstalled, rootHooked, stateVer };
    }
  };
